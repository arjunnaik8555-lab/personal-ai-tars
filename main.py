...
def main():
    ...
    speaker.speak("TARS systems online and operational.", wait=False)
    ...
    while True:
        try:
            if voice_mode:
                ...
                spoken_input = listener.listen(timeout=6, phrase_time_limit=12)
                ...
                if user_message.lower() in ["exit", "quit", "bye", "goodbye", "shut down"]:
                    ...
                    speaker.speak("Shutting down systems. Goodbye.", wait=True)
                    break
                elif user_message.lower() in ["text mode", "switch to text"]:
                    print("\n🔁 Switching to Text Mode.")
                    speaker.speak("Switching to text mode.", wait=True)
                    voice_mode = False
                    continue
                ...
                if lower_msg in ["v", "voice", "voice mode"]:
                    ...
                    speaker.speak("Voice mode activated. What can I do for you?", wait=True)
                    continue
            ...
            speaker.speak(response, wait=voice_mode)
